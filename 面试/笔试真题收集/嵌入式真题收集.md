...
### 1.2.13 空指针与野指针的区别 【问答题】

> **考点**：空指针是明确赋值为 NULL 的指针；野指针是指向不确定/非法内存的指针，来源有三种
> **注意**：野指针不等于空指针，野指针更危险，因为它可能看似合法但指向已被回收或随机的内存

空指针和野指针的区别是什么？

<details><summary>💡 答案提示</summary>
<ul>
<li><b>空指针（NULL Pointer）</b>：明确指向地址 0（NULL）的指针，表示"不指向任何有效对象"，是安全的初始化状态</li>
<li><b>野指针（Dangling Pointer）</b>：指向不确定/非法内存区域的指针</li>
</ul>
<p><b>野指针的三种典型产生方式：</b></p>
<ol>
<li><b>指针未初始化</b> —— 声明后未赋值，值为随机地址</li>
<li><b>free/delete 后未置 NULL</b> —— 内存已释放，指针仍保存原地址</li>
<li><b>指针指向的栈内存已被回收</b> —— 如返回局部变量的地址</li>
</ol>
</details>

---

### 1.2.14 访问空指针与野指针的后果 【问答题】

> **考点**：访问野指针是未定义行为，可能读到垃圾数据、破坏内存或崩溃；访问空指针通常触发段错误或 HardFault
> **注意**：空指针不是"安全的"，只是比野指针更容易被检测；嵌入式中无 MMU 的系统访问空指针可能不会崩溃

访问野指针会发生什么？访问空指针会发生什么？

<details><summary>💡 答案提示</summary>
<p><b>访问野指针：</b>行为是<b>未定义的（Undefined Behavior）</b>——</p>
<ul>
<li>可能读到垃圾数据（看似正常但数据错误，最危险）</li>
<li>可能破坏其他变量的数据（写操作踩了别人的内存）</li>
<li>可能触发段错误/HardFault 导致程序崩溃</li>
<li>嵌入式中可能导致系统跑飞、看门狗复位</li>
</ul>
<p><b>访问空指针：</b>行为也是<b>未定义的</b>，但通常表现更确定——</p>
<ul>
<li>在 Linux/Windows 等有 MMU 的系统上 → <b>段错误（Segmentation Fault）</b>，地址 0 所在页面被标记为不可访问</li>
<li>在无 MMU 的嵌入式系统（如 Cortex-M）上 → 可能触发 <b>HardFault</b>，也可能访问到地址 0 处的实际内容（中断向量表），导致不可预测的行为</li>
</ul>
</details>

---

### 1.2.15 指针释放后再次使用（Use-After-Free）【问答题】

> **考点**：free 后指针变为悬空指针（野指针），再次访问是未定义行为；对同一指针二次 free（Double Free）会破坏堆结构
> **注意**：防御性编程规范：free 后立即将指针置 NULL，后续误访问会触发空指针异常，便于定位问题

释放之后的指针再次调用会发生什么状况，如何避免？

<details><summary>💡 答案提示</summary>
<p><b>发生什么：</b>即 Use-After-Free，属于未定义行为。可能读到垃圾值、破坏堆管理结构、导致程序崩溃，也可能被利用进行安全攻击。对同一指针 <code>free</code> 两次（Double Free）还会破坏堆的内部数据结构。</p>
<p><b>如何避免：</b></p>
<pre><code>free(p);
p = NULL;  // 释放后立即置空，后续误访问会触发空指针异常，易于定位</code></pre>
</details>

---

### 1.2.16 指向整个数组的指针运算 【分析题】
...
### 1.2.17 指针函数与函数指针的区别 【问答题】

> **考点**：指针函数（返回指针的函数）本质是函数；函数指针（指向函数的指针）本质是指针；记忆看 `*` 与函数名的结合方式
> **注意**：函数指针在嵌入式中大量使用，如回调函数注册、状态机跳转表、驱动接口抽象等

指针函数与函数指针分别是什么？给出程序语句示例。

<details><summary>💡 答案提示</summary>
<p><b>指针函数：</b>返回值为指针的函数。本质是<b>函数</b>。</p>
<pre><code>int *func(int a, int b);  // 返回 int* 的函数</code></pre>
<p><b>函数指针：</b>指向函数的指针变量。本质是<b>指针</b>。</p>
<pre><code>int (*pfunc)(int, int);    // 指向 "参数为(int,int)、返回int" 的函数的指针
pfunc = add;               // 假设 int add(int a, int b) 已定义
int result = pfunc(1, 2);  // 通过函数指针调用函数</code></pre>
<p><b>记忆技巧：</b>看 <code>*</code> 和函数名的结合方式——</p>
<ul>
<li><code>int *func(...)</code> → <code>*</code> 修饰返回值，func 是函数 → <b>指针函数</b></li>
<li><code>int (*pfunc)(...)</code> → <code>*</code> 和 pfunc 结合在括号里，pfunc 是指针 → <b>函数指针</b></li>
</ul>
</details>

---

### 1.2.18 回调函数的定义与典型使用场景 【问答题】

> **考点**：回调函数通过函数指针实现，核心思想是控制反转——调用时机由框架/底层决定；嵌入式中大量用于中断回调、事件驱动、驱动抽象
> **注意**：回答必须先解释"什么是回调"，不能只说使用场景

请简述回调函数以及使用场合。

<details><summary>💡 答案提示</summary>
<p><b>什么是回调函数：</b>通过<b>函数指针</b>将一个函数作为参数传递给另一个函数，在特定条件满足时由被调用方"回过头来调用"传入的函数。核心是<b>控制反转</b>——调用时机由框架/底层决定，而非调用者。</p>
<pre><code>// 注册回调
void Timer_RegisterCallback(void (*callback)(void));

// 使用
void onTimeout(void) { /* 处理超时 */ }
Timer_RegisterCallback(onTimeout);  // 定时器到期时自动调用 onTimeout</code></pre>
<p><b>典型使用场景：</b></p>
<ol>
<li><b>中断回调</b> —— HAL 库中的中断处理（如 <code>HAL_UART_RxCpltCallback</code>）</li>
<li><b>定时器回调</b> —— 软件定时器到期回调、超时回调</li>
<li><b>事件驱动</b> —— 按键事件、通信数据接收完成、状态变化通知</li>
<li><b>驱动抽象</b> —— 上层注册处理函数，底层在合适时机调用</li>
</ol>
</details>
...
> **注意**：静态局部变量只初始化一次，后续调用保留上一次的值，这是笔试常考点；别漏答 static 变量

请简述静态变量、全局变量、局部变量的异同。

<details><summary>💡 答案提示</summary>
<table>
<tr><th>变量类型</th><th>存储位置</th><th>生命周期</th><th>可见性（作用域）</th></tr>
<tr><td>局部变量</td><td>栈</td><td>函数执行期间</td><td>仅函数/块内</td></tr>
<tr><td>静态局部变量（<code>static</code> 修饰局部）</td><td>.bss/.data 段</td><td>整个程序运行期间</td><td>仅函数/块内</td></tr>
<tr><td>全局变量</td><td>.bss/.data 段</td><td>整个程序运行期间</td><td>所有文件（可 extern）</td></tr>
<tr><td>静态全局变量（<code>static</code> 修饰全局）</td><td>.bss/.data 段</td><td>整个程序运行期间</td><td>仅当前文件（内部链接）</td></tr>
</table>
<p><b>关键点：</b><code>static</code> 的两种用法——</p>
<ol>
<li>修饰<b>局部变量</b>：延长生命周期（不改变作用域）</li>
<li>修饰<b>全局变量/函数</b>：限制作用域到当前文件（不改变生命周期）</li>
</ol>
</details>
