...
**内存碎片是指系统中存在无法被有效利用的内存空间。** 它会带来两方面影响：

**一是分配失败**：总的空闲内存可能足够，但由于空闲内存不连续，导致大块内存申请无法被满足，系统直接返回 NULL，严重时引发崩溃。

**二是分配变慢**：分配器在寻找合适内存块时，需要遍历大量细小的空闲碎片才能找到满足需求的块，分配时间变长且不可预测，在实时系统中这会破坏任务的时序要求。

#### 怎么产生的？

根本原因是**动态内存的频繁申请与释放**，且申请的大小不固定。

举个例子：

```
初始：[空闲 100KB]
申请 A=20KB：[A(20) | 空闲(80)]
申请 B=30KB：[A(20) | B(30) | 空闲(50)]
释放 A：    [空闲(20) | B(30) | 空闲(50)]
申请 C=60KB：失败！—— 虽然空闲总量70KB，但不连续
```
...
- 空闲内存散落在各处，总量够但不连续，无法分配大块内存
- 发生在堆管理层面
- 危害更大，嵌入式中最常见的问题

**内部碎片（Internal Fragmentation）**

- 分配器为了对齐或管理方便，给出的内存块比实际申请的大，多出来的部分浪费掉
- 例如申请 13 字节，实际分配 16 字节（4字节对齐），多出 3 字节无法被使用
- 发生在每一次内存分配内部

#### 真正避免碎片的做法（结合 FreeRTOS）

1. **系统初始化阶段一次性创建所有任务**，之后不再动态创建/删除
2. **用 FreeRTOS 的内存池思路**，自己封装固定大小块分配器
3. **避免在任务运行中频繁动态申请释放**，尤其是大小不固定的情况

####  FreeRTOS 的内存池思路

- **heap_3**：只是对标准库 malloc/free 加临界区保护的简单封装，内存来自**堆**，FreeRTOS 本身不参与管理。malloc 内部维护空闲块链表，每次申请时遍历链表找到合适的块切割给你，释放时尝试合并相邻块。由于每次申请大小不固定，长期使用后碎片越来越多，分配时间越来越长且不可预测。

- **heap_4**：FreeRTOS 自己实现的分配器，内存来自**全局数据区**的静态数组 `ucHeap`，在数组上自己维护空闲块链表，不依赖标准库。但同样支持任意大小的分配请求，空闲块会被切割成不同大小的碎片，本质上解决不了外部碎片。

- **自己写的内存池**：内存同样来自**全局数据区**的静态数组，强制每块大小固定，释放归还的块和新申请的块大小完全一致，可以直接复用，不存在切割动作，从根本上消除外部碎片。

内存池的实现比 heap_4 简单很多，可以参考 heap_4 的整体框架，比如用静态数组做内存来源、用链表管理空闲块，但把变长管理相关的逻辑全部去掉就行了

内存池的实现核心是一个二维静态数组，第一维是块的数量，第二维是每块的大小。定义一个只含 next 指针的结构体，复用每个块的头部空间来串链表。初始化时遍历二维数组的每一行，通过头插法把所有块串成一条空闲链表。申请时直接返回链表头块的行地址给调用者，释放时把该地址头插回链表即可。

```c
#define BLOCK_SIZE  32      // 每个块的大小（字节），根据业务中最大对象的sizeof来定
#define BLOCK_COUNT 20      // 块的总数量，根据业务中最大并发申请数来定

// 块头部结构体，只有一个next指针
// 作用：复用块头部空间来串链表，不需要额外的管理空间
typedef struct Block {
    struct Block *next;     // 指向下一个空闲块
} Block;

// 二维静态数组，内存来自全局数据区，编译期大小确定，不依赖堆
// 第一维：块的数量
// 第二维：每块的大小
static uint8_t pool_buf[BLOCK_COUNT][BLOCK_SIZE];
...
// 初始化：把所有块串成空闲链表
// 遍历每个块，把块头部强转为Block指针，头插法串成链表
// 初始化完成后 free_list → block19 → block18 → ... → block0 → NULL
void pool_init(void) {
    for (int i = 0; i < BLOCK_COUNT; i++) {
        Block *b = (Block *)pool_buf[i];  // 把块头部当作Block结构体来用
        b->next = free_list;              // 指向当前链表头
        free_list = b;                    // 自己成为新的链表头（头插法）
    }
}
...
void *pool_alloc(void) {
    if (free_list == NULL) return NULL;   // 池子已用完，返回NULL

    Block *b = free_list;                 // 取出链表头的块
    free_list = b->next;                  // 链表头更新为下一个块
    return (void *)b;                     // 返回给调用者使用
    // 注意：此时块头部的next指针会被调用者的业务数据覆盖，没关系
    // 因为归还时会重新写入next指针
}
...
void pool_free(void *p) {
    Block *b = (Block *)p;                // 把归还的块头部重新当作Block结构体
    b->next = free_list;                  // 指向当前链表头（重新写入next指针）
    free_list = b;                        // 自己成为新的链表头
}
```
...
**先说裁剪方式**：FreeRTOS 的裁剪本质上是通过修改 `FreeRTOSConfig.h` 里的宏定义来实现的，CubeMX 只是提供了一个图形化界面，最终还是生成这个配置文件。

裁剪大致可以分三类：

**功能模块**：用不到的直接关掉，最典型的是软件定时器（`configUSE_TIMERS`），关掉能省一个任务和一个队列，比较划算。其他像计数信号量、递归互斥量、事件组、协程这些用不到就关。

**调试统计**：运行时统计、Trace 支持、栈溢出检测这些在量产时都可以关掉，开发阶段用来调试，上线后没必要保留。

- **Trace 支持**，开启后可以调用 `vTaskList()` 打印出所有任务的状态和栈水位线，也就是每个任务历史上栈最多用到多少，这个对我设置每个任务的栈大小很有参考价值，不用靠猜。
- **运行时统计**，可以统计每个任务实际占用了多少 CPU 时间，帮助我判断优先级设置是否合理，比如某个任务 CPU 占用异常高，可能是优先级设高了或者逻辑有问题。
- **栈溢出检测**，前两个是事后统计，这个是实时报警，一旦任务栈真的溢出了会立刻触发钩子函数，两个配合用更保险。

**资源数量**：优先级数量（`configMAX_PRIORITIES`）没必要设很大，够用就好，越大越费 RAM；任务名长度也可以缩短；堆大小按实际需求裁。

**最后一句总结**：必须保留的核心是调度器本身、Tick 配置、中断优先级配置这几个，其余大部分功能模块都是可选的，按项目实际用到什么开什么就行。
...
最常用、最简单的软件调试手段。通过串口把变量值、运行状态、错误信息打印出来观察。
...
**局限：** 串口打印本身会占用 CPU 时间，在时序要求严格的场合可能影响程序行为，且无法调试串口本身的问题。

------

## 仿真器调试(JTAG/SWD 调试)

通过调试器（如 ST-Link、J-Link）连接芯片，配合 IDE（Keil、VSCode）进行在线调试。
...
**局限：** 打断点会暂停程序，不适合调试实时性强的场景，比如中断处理、电机控制等。

------

## LED 灯调试

用 GPIO 控制 LED 亮灭来指示程序运行状态，是最原始也最可靠的调试手段。
...
**局限：** 信息量极少，只能表达是/否，无法观察具体数值。

------

## 逻辑分析仪

采集多路数字信号，还原时序波形，可以解析 UART、SPI、I2C、CAN 等协议内容。

**适用场景：**

- 调试通信协议，验证时序是否正确
- 观察多路信号之间的时序关系
- 排查 SPI/I2C 通信异常，查看实际收发的数据内容
- 中断响应时间、任务切换时间的测量

**局限：** 只能采集数字信号，无法观察模拟信号的电压波形。

------

## 示波器

观察模拟信号的真实电压波形，是硬件调试的核心工具。

**适用场景：**

- 观察电源纹波、信号质量
- 测量 PWM 波形的频率、占空比是否正确
- 排查信号上升沿/下降沿是否正常，有无振铃、过冲
- 硬件电路不工作时，确认信号是否到达芯片引脚

**局限：** 主要用于硬件层面，无法直接解析协议内容，通道数少，不适合同时观察多路数字信号。

# 硬件存储类型

## FLASH 里存什么

程序代码和常量数据都存在 FLASH 里，具体包括：

- FreeRTOS 的内核代码、任务函数代码
- 中断向量表
- const 修饰的常量，比如配置表、字库
- 需要掉电保存的用户数据，比如设备参数、PID 参数、校准值

芯片上电后从 FLASH 读取指令开始执行，也就是说 FreeRTOS 的调度器、任务代码全部存在 FLASH 里。

------

## RAM 里存什么

程序运行时产生的所有数据都在 RAM 里，具体包括：

- 全局变量、静态变量
- 每个 FreeRTOS 任务的**任务堆栈**，每创建一个任务都要从 RAM 里划出一块空间作为该任务的栈
- FreeRTOS 的内核对象，比如任务控制块 TCB、队列、信号量、互斥锁
- heap_4 的静态数组 `ucHeap` 也在 RAM 里，pvPortMalloc 申请的内存都从这里来
- 我们自己写的内存池的二维静态数组同样在 RAM 里

RAM 是嵌入式中最紧张的资源，STM32 典型只有几十KB，所以 FreeRTOS 任务堆栈大小要仔细规划，开太大浪费，开太小会栈溢出。

------

## ROM 呢

现代嵌入式中 ROM 基本已被 FLASH 取代，但有一个地方还能见到它的身影：STM32 内部有一块出厂固化的 **Bootloader 存在系统存储区**，这块区域是只读的，本质上就是 ROM 的概念，用于 ISP 串口下载程序。
...
上电启动
   ↓
从 FLASH 读取程序代码（FreeRTOS内核、任务函数）
   ↓
运行时数据放入 RAM（任务堆栈、TCB、队列、内存池）
   ↓
需要掉电保存的数据写回 FLASH（设备参数、校准值）
```

三者的关系就是：**FLASH 负责存，RAM 负责跑，ROM 在现代嵌入式里基本退出舞台被 FLASH 取代。**

# FreeRTOS中断优先级

Cortex-M 使用 NVIC（嵌套向量中断控制器）管理中断，优先级用寄存器中的若干位来表示，STM32 通常使用 **4位**，也就是 0～15 共 16 个优先级，**数值越小优先级越高**。

这4位又分为两部分：

- **抢占优先级**：决定中断能不能打断正在执行的其他中断（中断嵌套）
- **子优先级**：抢占优先级相同时，决定谁先执行，但不能互相打断

FreeRTOS 官方推荐使用**分组4**，也就是 4 位全部用作抢占优先级，子优先级为 0。

原因是 FreeRTOS 的中断管理只关心抢占优先级，子优先级对它没有意义，全部用作抢占优先级逻辑最清晰，不容易出错。

STM32 的 HAL 库初始化时默认也是设置为分组4。
...
- 可以调用 FreeRTOS 的 ISR 安全 API，比如 `xQueueSendFromISR`、`xSemaphoreGiveFromISR`
- 受 FreeRTOS 内核管理，调用 `taskENTER_CRITICAL()` 可以屏蔽这类中断

**优先级数值 < 5（高优先级中断）**

- 不能调用任何 FreeRTOS API
- 不受 FreeRTOS 内核管理，`taskENTER_CRITICAL()` 屏蔽不了这类中断
- 实时性最高，适合对时序要求极严格的场景，比如电机控制、编码器计数

## PWM 和按键中断优先级设置举例

------

前提：FreeRTOS 推荐分组4，4位全部是抢占优先级
...
```c
HAL_NVIC_SetPriority(TIM1_UP_IRQn, 3, 0);  // 抢占优先级3，小于5
HAL_NVIC_EnableIRQ(TIM1_UP_IRQn);
```

数值 3 < 5，属于高优先级中断，FreeRTOS 临界区屏蔽不了它，任何时候都能立即响应，保证 PWM 时序精度。

中断处理函数里**只操作硬件寄存器**，不调用任何 FreeRTOS API：

```c
void TIM1_UP_IRQHandler(void) {
    // 直接操作寄存器，不能调用 FreeRTOS API
    __HAL_TIM_CLEAR_FLAG(&htim1, TIM_FLAG_UPDATE);
    TIM1->CCR1 = new_duty;  // 直接更新占空比
}
```
...
```c
HAL_NVIC_SetPriority(EXTI0_IRQn, 6, 0);  // 抢占优先级6，大于5
HAL_NVIC_EnableIRQ(EXTI0_IRQn);
```

数值 6 ≥ 5，属于低优先级中断，受 FreeRTOS 管理，可以调用 FromISR 系列 API：
...
中断是硬件触发的，不是代码主动调用的，CPU 响应中断时直接跳转到中断向量表里对应的地址执行，没有调用者，自然也没有人传参数进来，也没有人接收返回值。

### 如何与外界通信

- 裸机情况：使用全局变量
  - 最简单直接的方式，中断修改全局变量，主循环读取，注意全局变量必须加 **volatile**，告诉编译器这个变量可能在意料之外被修改，不要对它做优化缓存。

- FreeRTOS 情况：使用FromISR系列API
  - 通过队列、信号量、事件组等方式通知任务，让任务来处理具体逻辑。

# 配置一个引脚输入输出功能的流程

## 输出配置（以点亮 LED 为例）

1. **使能时钟**：GPIO 挂载在 APB 总线上，使用前必须先开时钟，否则寄存器写入无效
2. **配置模式**：设置为推挽输出或开漏输出
3. **配置速度**：设置 GPIO 翻转速率，低速/中速/高速
4. **操作引脚**：通过 HAL_GPIO_WritePin 或直接操作 ODR 寄存器控制高低电平
...
GPIO_InitStruct.Mode  = GPIO_MODE_OUTPUT_PP;  // 2. 推挽输出
GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;  // 3. 低速
HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);       // 4. 初始化
...
1. **使能时钟**：同上
2. **配置模式**：设置为输入模式
3. **配置上下拉**：根据硬件电路决定上拉/下拉/浮空
4. **读取引脚**：通过 HAL_GPIO_ReadPin 读取电平

```c
__HAL_RCC_GPIOC_CLK_ENABLE();          // 1. 使能时钟

GPIO_InitTypeDef GPIO_InitStruct = {0};
GPIO_InitStruct.Pin  = GPIO_PIN_13;
GPIO_InitStruct.Mode = GPIO_MODE_INPUT;       // 2. 输入模式
GPIO_InitStruct.Pull = GPIO_PULLUP;           // 3. 上拉
HAL_GPIO_Init(GPIOC, &GPIO_InitStruct);       // 初始化

uint8_t state = HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_13);  // 4. 读取电平
```
