...
#### 旁证：JL 官方芯片生态的全 2KB 格局

除实验证据外，`nandflash.c` 的芯片支持表本身就是一个静态线索。JL 官方支持的 7 款 SPI NAND：
...
**全部是 2KB page，没有一款 4KB。** `ftl.a` 就是为这 7 款芯片编译的，其内部代码路径只被 2KB page 的几何参数执行过。从工程角度看，开发者没有动机去适配一个不存在的芯片型号——即使 `ftl_nand_flash.page_size` 字段在接口层面可配置，实现层面的地址映射、spare 区切分、元数据布局等逻辑中嵌入 2KB 的硬编码假设是自然的结果。GD5F4GM7UE（4KB page）是后来新增的 case 分支（2.3 节），`ftl.a` 从未为它编译或测试过。

#### 综合结论

三组测试 + 芯片生态分析，排除了所有替代解释，精确定位到根因：
