...
```c
#define TCFG_CHARGE_POWERON_ENABLE  1   // sdk_config.h:32，已为 1 ✅
```
...
#define TCFG_NANDFLASH_DEV_ENABLE   1   // board_ac701n_demo_cfg.h:98，改为 1
```
...
| 步骤 | 文件 | 操作 | 说明 |
|------|------|------|------|
| 1 | `sdk_config.h:32` | **确认** `TCFG_CHARGE_POWERON_ENABLE = 1` | 插电开机，PC 模式硬前提 |
| 2 | `sdk_config.h:261` | `TCFG_APP_PC_EN` 改为 `1` | 总开关 |
| 3 | `sdk_config.h:83` | 确认 `TCFG_SD0_ENABLE = 1` | SD NAND 已启用（当前默认） |
| 4 | `sdk_config.h:108-116` | 确认四个 `TCFG_USB_SLAVE_*` 宏为 `1` | USB 子功能（当前默认全开） |
| 5 | 编译烧录 | — | 插 USB 线到电脑即可自动进入 PC 模式 |
...
PC 模式开启时，USB OTG Slave 的检测逻辑会干扰 EFF（音效验证测试）的启动流程，因此被临时关闭。
...
| `apps/earphone/board/br28/board_ac701n_demo_cfg.h` | 板级配置——`TCFG_NANDFLASH_DEV_ENABLE`、GPIO 冲突断言、`TCFG_SD0_FORMAT_ON_BOOT` |
...
| 步骤 | 检查点 | 方法 |
|------|--------|------|
| 1 | Flash 是否挂载 | 确认 `dev_manager` 日志中 `"sd0"` 或 `"nandflash_ftl"` 已成功 mount；若未挂载，检查 `dev_reg.c` 注册和 `TCFG_SD0_ENABLE`/`TCFG_NANDFLASH_DEV_ENABLE` |
| 2 | `msd_register_disk` 是否调用 | 在 `task_pc.c:usb_start()` 中加日志，确认对应的 `msd_register_disk("sd0"/"nandflash_ftl", NULL)` 被执行 |
| 3 | 文件系统是否 FAT32 | USB MSD 暴露的是裸 block device，但文件系统层必须已格式化为 FAT32，否则电脑提示"需要格式化" |
| 4 | GPIO 是否冲突 | 确认未同时启用 SD0 和 SPI NAND（编译时 `#error` 会拦截，但绕过的情况需要检查） |
...
| 步骤 | 检查点 | 方法 |
|------|--------|------|
| 1 | `TCFG_APP_PC_EN` 是否为 1 | 检查 `sdk_config.h:261` |
| 2 | OTG 检测是否正常 | 在 `app_pc_check()` 中打日志，看 `usb_otg_online(0)` 返回值是否为 `SLAVE_MODE` |
| 3 | `TCFG_PC_BACKMODE_ENABLE` | 此宏若为 1，`app_pc_check()` 强制返回 false |
| 4 | USB 线缆是否为数据线 | 充电线（D+/D- 短接）无法触发 OTG Slave 检测 |
