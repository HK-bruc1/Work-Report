...
void rdx_rtc_init(void)
{
    ...
    if (!rdx_rtc_sys_time_valid(&current_time)) {
        ...
        rdx_rtc_timestamp_to_timezone_string(
            rdx_rtc_sys_time_to_timestamp(&current_time), 
            RDX_RTC_TIMEZONE_OFFSET_SEC, utc_buf);
...
graph TB
    ...
    subgraph "断电开机"
        B1[硬件RTC<br/>无效] -->|回退| B2[VM备份]
        B2 -->|恢复| B1
    end

    subgraph "APP设置"
        C1[APP下发] -->|双写| C2[硬件RTC]
        C1 -->|双写| C3[VM备份]
    end
...
|------|---------|---------|
| init | 硬件正常走时 | 硬件当前时间 |
| set | APP下发时间设置 | APP下发的时间戳 |
| get | 首次检测到有效硬件时间 | 硬件当前时间 |
