...
官方文档里虽然区分了录音模式、录音翻译模式、音视频翻译模式、通话翻译模式，但从流程角度看，本质上只是“什么音源接到 `AI_TX`、回来的音频落到哪里”这两个开关不同。为了不冲淡本文主线，这里不展开各模式细节，统一放到文末附录“JL 原生 AI 翻译完整链路”中收口。
...
这里最好再把“节点名”和“真实代码入口”区分开来：`NODE_UUID_SINK_DEV1` 是流程图 / pipeline 里的抽象输出节点名，但真正把它接到 RDX 上的是 `audio/framework/nodes/sink_dev1_node.c` 这份手写节点代码。当前路径里，`translation_ear_recoder.c` 会先通过 `NODE_IOC_SET_FMT` 把编码格式、采样率和声道模式写给 `SINK_DEV1`；随后 `sink_dev1_ioctl()` 再负责 `OPEN_IPORT / START / STOP / NEGOTIATE / SET_FMT` 这些运行时控制；真正拿到编码帧时，才在 `sink_dev1_run()` 里调用 `rdx_record_run_data_handle()`。因此，**`SINK_DEV1` 是 JL 到 RDX 的抽象边界名，而 `sink_dev1_run() -> rdx_record_run_data_handle()` 才是当前代码里的显式交接入口。**
...
但这不等于“`rdx_record_run_data_handle()` 之前的部分都只是 JL 可视化工具里定义好的数据流程”。更准确的拆法是：`.x6flow` 主要负责描述节点拓扑和 pipeline UUID；而 `translation_ear_recoder.c`、`sink_dev1_node.c`、`source_dev1` 相关节点实现则负责运行时的参数协商、节点启停、帧拉取和出口绑定。也就是说，**上游既有流程图层，也有手写运行时代码层**；只是到了 `sink_dev1_run() -> rdx_record_run_data_handle()` 这里，JL 音频流和 RDX 上传链的责任边界才变得最清晰。
...
## 八、附录：JL 原生 AI 翻译完整链路

这一节只把 JL 原生 AI 翻译能力本身补成闭环，不再展开当前 RDX 的桥接、协议和业务状态机。这里分两种证据口径：`ai_voice_recoder.c`、`ai_tx_node.c`、`mic_rec.c` 来自当前工作区，可直接验证；`ai_recorder.c` 和 `AI_RX` 下行节点的具体实现当前工作区未直接暴露，因此以下涉及控制状态机和下行接收边界的表述，统一沿用前文已经引用过的官方《AI翻译》文档口径，只补到框架边界，不把它们写成“当前仓库源码逐行可证”的事实。

### 1. `ai_recorder.c` 的控制职责与状态机

如果把 `ai_recorder.c` 看作 JL 原生 AI 翻译的控制面，它负责的不是翻译算法本身，而是把 `start / pause / resume / stop` 这些业务动作编排到录音上行链和回放下行链上。结合当前工作区能直接看到的 `mic_rec.c` 控制层，这个状态机可以收敛成下表：

| 控制动作 | 状态变化 | 当前可确认的控制动作 | 说明 |
|----------|----------|----------------------|------|
| `start` | `idle -> running` | 退出 sniff、必要时挂起 A2DP/TWS 解码、初始化 `speech_cbuf`、调用 `ai_voice_recoder_open()`、进入 busy 状态 | 这一段可以直接从 `mic_rec.c` 的 `ai_mic_rec_start()` 看见 |
| `pause` | `running -> pause` | 暂停当前 AI 会话的采集/播放推进，但不改变“仍处于 AI 翻译会话”这一语义 | 当前工作区未直接暴露 `ai_recorder.c` 源码，这里只按官方控制语义收口 |
| `resume` | `pause -> running` | 恢复同一条 AI 会话的上行采集，若模式需要，也恢复 `AI_RX` 对应的下行播放 | 同上，当前工作区只能确认它是控制层动作，不能展开到具体实现细节 |
| `stop` | `running/pause -> idle` | 调用 `ai_voice_recoder_close()`、释放 `speech_cbuf`、恢复时钟、恢复 A2DP/TWS 播放链 | 这一段可以直接从 `mic_rec.c` 的 `ai_mic_rec_close()` 看见 |

如果只保留最核心的控制主线，JL 原生 AI 翻译的控制面可以简化成：

```text
start -> 打开 AI 上行链（必要时带下行链） -> running
pause -> 临时停采/停播，但保留会话语义
resume -> 恢复同一会话的采集/播放
stop -> 关闭链路并回到 idle
```

### 2. `ai_voice_recoder.c` 的上行数据流完整配置

当前工作区里，JL 原生 AI 翻译上行链最直接的源码锚点就是 `ai_voice_recoder.c`。它把“AI 录音上传”落成一条标准 `jlstream`：
...
| 配置项 | 当前可确认的内容 | 作用 |
|--------|------------------|------|
| pipeline 入口 | `STREAM_EVENT_GET_PIPELINE_UUID("ai_voice")` | 取 JL 原生 AI 语音/翻译录音流程 |
| source 节点 | `NODE_UUID_ADC` | 从本地麦克风 ADC 起流 |
| encoder 节点 | `NODE_UUID_ENCODER` | 负责 OPUS 或 SPEEX 编码 |
| OPUS 关键参数 | `bit_rate = 32000`、`sample_rate = 16000`、`format = 0`、`complexity = ai_type` | 当前源码里 AI 录音默认走 16kHz OPUS 上传 |
| 输出节点 | `NODE_UUID_AI_TX` | 把编码帧交给 AI 协议出口 |
| source 中断点 | `NODE_UUID_SOURCE` 私有参数 `320` | 配置 ADC 侧取数粒度 |
| scene | `STREAM_SCENE_AI_VOICE` | 把整条流标成 AI 语音/翻译场景 |

再往节点实现里看，`ai_tx_node.c` 还能补出 `AI_TX` 的边界语义：它在协商阶段把输出采样率收敛到 `16000`，并把声道模式收敛到 `AUDIO_CH_MIX`；真正收到编码帧后，再通过 `rec_enc_output()` 把数据交给外部协议层。因此，从 JL 原生角度看，`AI_TX` 是一个**上传出口节点**，不是翻译算法节点。

### 3. 下行链路 `AI_RX` 的节点边界与播放路径

当前工作区没有直接暴露 `AI_RX` 对应的节点枚举或 `ai_rx_node.c` 源码，所以这里不能把下行链写成“本地源码逐行已证”的事实；但按前文已经引用过的官方《AI翻译》文档口径，JL 原生 AI 翻译的下行链可以稳定收敛成下面这条路径：

```text
APP/协议返回音频 -> AI_RX -> DECODER -> DAC 或 ESCO_TX
```

这里有三个边界要点：

1. `AI_RX` 和 `AI_TX` 一样，本质上是边界节点，不承担翻译语义本身；真正的翻译或转写仍在 APP / 协议侧完成。
2. `AI_RX` 之后仍然回到 JL 平台自己的通用播放链，也就是“接收 -> 解码 -> 播放”这套播放器路径，而不是单独另起一套 AI 专用播放框架。
3. 最终播放出口由场景决定：本地播报类场景通常落到 `DAC`，通话类场景则可以按配置落到 `ESCO_TX` 或其他通话播放/回送路径。

因此，如果把 JL 原生 AI 翻译完整链路写成一句话，就是：**设备侧负责采集、编码、上传、接收、解码和播放；APP / 协议侧负责真正的翻译语义处理。**

### 4. 各翻译模式的差异

把官方几种模式按“上行音源是什么、是否需要 `AI_RX`、回来的音频落到哪里”这三个维度压缩后，可以收成下表：

| 模式 | 上行主音源 | 是否需要 `AI_RX` 下行 | 典型播放出口 | 说明 |
|------|------------|------------------------|--------------|------|
| 录音模式 | `ADC` | 否 | 无 | 只做本地采集、编码、上传，对应最纯粹的 `ADC -> AI_TX` |
| 录音翻译模式 | `ADC` | 是 | `DAC` | 在录音模式基础上增加返回音频的本地播放 |
| 音视频翻译模式 | 媒体播放流，通常可概括为 `A2DP_RX` 一类源头 | 是 | `DAC` | 上行送翻译的不再是 MIC，而是当前媒体内容 |
| 通话翻译模式 | 通话相关音源，通常可概括为 `ESCO_RX` 或其通话上下行组合 | 是 | `DAC` 或 `ESCO_TX` | 本质上仍是“通话音源上送 + 翻译结果回放/回送”，只是播放出口更依赖场景配置 |

如果只抓住模式差异的最小抽象，可以把它总结成两句话：

1. **录音模式和录音翻译模式的区别，在于有没有 `AI_RX` 下行回放。**
2. **音视频翻译模式和通话翻译模式的区别，在于送进 `AI_TX` 的主音源从 `ADC` 换成了媒体流或通话流。**
