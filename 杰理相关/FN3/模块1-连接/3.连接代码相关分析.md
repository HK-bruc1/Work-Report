...
当TWS连接成功后，设备会尝试连接手机。具体逻辑和调用关系如下：

- **主从角色判定**： 在[bt_tws.c](javascript:void(0))的`TWS_EVENT_CONNECTED`事件处理中：
  - 当前设备是主耳且未连接手机时，会调用连接函数
...
- **关键函数调用链**： `bt_tws_connect_and_connectable_switch()` → `bt_wait_connect_and_phone_connect_switch(0)`
  - 最终调用栈：`bt_wait_connect_and_phone_connect_switch()` → `user_send_cmd_prepare(USER_CTRL_START_CONNEC_VIA_ADDR)`
- **自动连接机制**：
  - 主耳会继承预设的自动连接时间参数，触发定时连接流程
...
→ bt_tws_connect_and_connectable_switch() 
  → bt_wait_connect_and_phone_connect_switch(0) 
    → user_send_cmd_prepare(USER_CTRL_START_CONNEC_VIA_ADDR)
    → sys_timeout_add() 创建连接定时器
```

- 该逻辑实现在`bt_tws.c`的`TWS_EVENT_CONNECTED`事件处理分支中，通过`bt_tws_connect_and_connectable_switch()`函数控制手机连接行为。
