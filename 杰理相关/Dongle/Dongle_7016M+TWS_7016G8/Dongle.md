...
1. 耳机端使用demo板，没复现到延迟波动大的情况，使用样机板子，复现到延迟波动大情况
2. 通过打印分析，是因为样机板子上在低延迟状态下，很容易播空，触发自适应动态延迟，把延迟调整到了550ms，然后再开始根据缓存情况下降到30ms
3. demo板测试就不会播空，延迟可以稳定，对比就是demo板的RF指标是比较好的状态
4. 延迟越低滴哦RF性能要求越高，建议调整耳机和dongle的RF性能到优良水平，并保留调试的RF数据表单，后再测试延迟性能
