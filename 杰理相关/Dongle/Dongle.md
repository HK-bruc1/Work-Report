...
1. 实际样机RF性能不算差，但是因为demo板和裸板天线外漏，RF性能上多多少少是demo板≈>样机裸板>样机整机；
2. 由于软件上走的是2.4g经典蓝牙的链路，设置30ms延迟很极限，在demo板和裸板上，RF最优的情况下测试，偶尔才有一次延迟增大到500ms的情况，而整机RF略微差些，频率就会大些；
3. 延迟增大是因为耳机端蓝牙收到的有效数据，不能满足在30ms内稳定播放了，数据准备播空耳机就会触发自适应延迟策略，把延迟设置到自适应最大延迟550ms，目的是避免蓝牙收数不好的时候，延迟太低会频繁卡顿，收数稳定后延迟会慢慢下降到目标的30ms；
4. 把延迟设定为40ms，则样机整机也能稳定在40ms左右播放，所以需要放宽一下延迟留一些余量；
5. 另外软件上开启tws配对超时后配对新耳机，或者一拖二连接等，开启蓝牙page或者page scan这些操作的话，也会占用蓝牙带宽，影响一点播歌收数，根据实际需求测试取舍

## 普通按键UI在dongle使用时会有问题
