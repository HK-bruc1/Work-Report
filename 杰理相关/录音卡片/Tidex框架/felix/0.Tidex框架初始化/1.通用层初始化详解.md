...
| 振动活跃   | `s_is_vibrate_active_fn && s_is_vibrate_active_fn()`         | 马达运转时保持供电稳定（`#if TDX_HAS_VIBRATE` 条件编译） |
...
`tdx_app_emmc_poweroff_check_timer_stop()` 仅在 `tdx_app_power.c` 内部就有 4 处生命周期调用，此外还被 `tdx_app.c`（WiFi 开关、空闲处理）、`tdx_app_record.c`（存储挂载）、`tdx_charge.c`（充电事件）、`tdx_dut.c`（DUT 模式）等模块调用，全项目合计约 10 处。以下列出核心的 4 处生命周期调用点：
...
| `on_state_changed`      | `record_ops_on_state_changed`      | —                                    | 录音状态变化回调（`old_state, new_state, scene` 三参数，控制 TWS、UI、振动、DAC 关电） |
...
这是录音平台操作集中最复杂的回调。SDK 核心在录音状态变化时调用它，回调签名为：

```c
void (*on_state_changed)(u8 old_state, u8 new_state, u8 scene);
```

应用层根据 `new_state` 完成大量副作用操作：
