...
![image-20250715164151945](./新架构.assets/image-20250715164151945.png)
...
- 连字符 `-` 在C语言中是减号运算符
- 如果允许在标识符中使用 `-`，编译器无法区分这是标识符还是减法运算

**宏也没办法转换。好像可以，但是vscode不支持高亮。**

### bug5

使用vscode修改客户的`customer.conf`客户多起来时，同名文件太多，很容易改错。

### copy脚本以及最终生效的文件位置

![image-20250715180324621](./新架构.assets/image-20250715180324621.png)

![image-20250715180443699](./新架构.assets/image-20250715180443699.png)

**这四个文件都是从tools路径下copy过来的。按照以往的经验生效还是在tools在，copy到这个地方可能为了方便下载。**
...
@echo ********************************************************************************
set CUSTOMER_PATH=%1
copy .\customer\%CUSTOMER_PATH%\customer.conf .\apps\earphone\board\br56
copy .\customer\%CUSTOMER_PATH%\sdk_config.h .\apps\earphone\board\br56
copy .\customer\%CUSTOMER_PATH%\sdk_config.c .\apps\earphone\board\br56

copy .\customer\%CUSTOMER_PATH%\anc_coeff.bin .\cpu\br56\tools
copy .\customer\%CUSTOMER_PATH%\anc_gains.bin .\cpu\br56\tools
copy .\customer\%CUSTOMER_PATH%\cfg_tool.bin .\cpu\br56\tools\download\earphone

copy .\customer\%CUSTOMER_PATH%\jlstream_node_cfg.h .\apps\earphone\board\br56

xcopy .\customer\%CUSTOMER_PATH%\src  ..\src  /E /Y /I

xcopy /y /s /e .\customer\%CUSTOMER_PATH%\tone_en   .\cpu\br56\tools\download\earphone\tone_en
```

新`cpu\br56\tools\download\earphone\download.bat`
...
REM copy %PROJ_DOWNLOAD_PATH%\*.bin . 这是从原来的output中复制二进制文件到当前目录，说明已经生成了，不影响

REM tone_xx.cfg文件的处理,会复制到当前目录，copy脚本不用写了。
if exist %PROJ_DOWNLOAD_PATH%\tone_en.cfg copy %PROJ_DOWNLOAD_PATH%\tone_en.cfg .	
if exist %PROJ_DOWNLOAD_PATH%\tone_zh.cfg copy %PROJ_DOWNLOAD_PATH%\tone_zh.cfg .

REM 当前目录哪里可能会有这两个文件？神经
if exist sdk_config.h del sdk_config.h
if exist sdk_config.c del sdk_config.c
...
这个bin文件只有通过可视化工具编译一遍才会产出，不然蓝牙名等配置不生效。
...
这里的 `isd_download.exe` 工具负责生成 `cfg_tool.bin` 文件，它作为 `-res` 参数的输入。

`isd_download.exe` 工具实际上是**使用**`cfg_tool.bin` 文件，而不是生成它。

`isd_download.exe` 是固件打包工具，它需要 `cfg_tool.bin` 作为输入，而不是生成它。你需要先找到或生成 `cfg_tool.bin` 文件，然后才能使用 `isd_download.exe` 进行固件打包。

从这个批处理脚本中**无法直接看到** `cfg_tool.bin` 的生成过程。这个脚本主要是固件打包和下载的流程，`cfg_tool.bin` 应该是在**更早的编译阶段**生成的。

**那为什么我看到有工具可以直接修改配置并保存为cfg_tool.bin？不需要编译呢？**

这说明 `cfg_tool.bin` 实际上是一个**标准格式的配置文件**，而不是编译生成的代码

**那说明工具可以把配置写进cfg_tool.bin然后在最后把打包进最终固件？**

**配置写入阶段**

```bat
# 使用配置工具修改/生成 cfg_tool.bin
config_tool.exe --modify cfg_tool.bin --set "param1=value1" --set "param2=value2"
# 或者
cfg_editor.exe cfg_tool.bin  # 图形界面修改
```
...
```bat
# 脚本中的关键命令
..\..\isd_download.exe ..\..\isd_config.ini -tonorflash -dev br56 \
  -boot 0x100000 -div8 -wait 300 \
  -uboot ..\..\uboot.boot \
  -app ..\..\app.bin \
  -tone %TONE_FILES% \
  -res cfg_tool.bin \      # ← 这里打包配置文件
  ..\..\p11_code.bin stream.bin %CONFIG_DATA% \
  %KEY_FILE% %FORMAT% -key %CHIPKEY% -format all
```

**最终固件生成**

```bat
各种二进制文件 + cfg_tool.bin → jl_isd.fw → update.ufw
```
...
1. **宏定义仍然保留**：用来在编译时确定是哪个客户
2. **功能参数从宏中解放**：不再硬编码在 `#ifdef` 中，而是放在配置表里
3. **业务代码大幅简化**：从满屏的宏判断变成简单的配置查询
...
```c
// 所有地方都是这样
const customer_config_t* config = get_customer_config();
// 然后直接用 config->xxx
```

既然你们已经有脚本基础设施，这个方案特别适合，因为：

1. **复用现有流程**：仍然是 conf → 脚本 → 头文件
2. **渐进式迁移**：可以一个功能一个功能地迁移
3. **向后兼容**：原有宏定义可以保留作为过渡
...
```c
当前方式:
conf配置 → 脚本 → 宏定义头文件 → 业务代码中大量#ifdef
```

#### 新方案流程

**配置文件 (customer_config.json)**

```c
{
  "customers": {
    "YYSX_S30": {
      "macros": ["_YYSX_S30_Left", "_YYSX_S30_Right"],  // 原有宏定义
      "bt_disconnect": {
        "master_led_state": "LED_STA_RED_BLUE_FAST_FLASH_ALTERNATELY",
        "slave_led_state": "LED_STA_RED_FLASH_1TIMES_PER_5S",
        "use_timeout": false
      }
    },
    "YYSX_H28": {
      "macros": ["_YYSX_H28_Left", "_YYSX_H28_Right"],
      "bt_disconnect": {
        "master_led_state": "LED_STA_RED_BLUE_FAST_FLASH_ALTERNATELY", 
        "slave_led_state": "LED_STA_RED_FLASH_1TIMES_PER_5S",
        "use_timeout": false
...
// 新的代码 - 完全消除宏判断
const customer_config_t* config = get_customer_config();
if (config->bt_disconnect.use_timeout) {
    // 默认处理
} else {
    if (tws_api_get_role() == TWS_ROLE_MASTER) {
        led_ui_set_state(config->bt_disconnect.master_led_state, DISP_CLEAR_OTHERS);
    } else {
        led_ui_set_state(config->bt_disconnect.slave_led_state, DISP_CLEAR_OTHERS);
    }
}
```
...
- **编译时确定**: 宏仍然用于确定当前是哪个客户
- **配置表驱动**: 功能参数从配置表读取，而不是硬编码
- **业务代码清洁**: 消除所有功能相关的宏判断

#### 具体执行机制

1. 编译时
   - 脚本根据宏定义生成 `CURRENT_CUSTOMER_ID`
   - 配置表编译到固件中
2. 运行时
   - `get_customer_config()` 根据 `CURRENT_CUSTOMER_ID` 返回对应配置
   - 业务代码直接读取配置参数，无需宏判断
3. 性能
   - 客户ID在编译时确定，运行时是常量
   - 配置查找会被编译器优化为直接内存访问
...
```c
void handle_battery_low(void) {
#if defined(_YYSX_S30_Left) || defined(_YYSX_S30_Right)
    led_ui_set_state(LED_STA_RED_FLASH_3TIMES, DISP_CLEAR_OTHERS);
    play_tone(TONE_BATTERY_LOW_CHINESE);
#elif defined(_YYSX_H28_Left) || defined(_YYSX_H28_Right)  
    led_ui_set_state(LED_STA_RED_SLOW_FLASH, DISP_CLEAR_OTHERS);
    play_tone(TONE_BATTERY_LOW_ENGLISH);
#else
    led_ui_set_state(LED_STA_RED_FLASH_1TIMES, DISP_CLEAR_OTHERS);
#endif
}
```
...
void handle_battery_low(void) {
    const customer_config_t* config = get_customer_config();
    led_ui_set_state(config->battery.low_led_state, DISP_CLEAR_OTHERS);
    play_tone(config->battery.low_tone);
}
```

配置在JSON中定义，脚本自动生成对应的C代码。

**需要python脚本，能不能做一个工具？不然每一个电脑都要下载python环境？**
