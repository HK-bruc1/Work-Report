...
- 一般是带霍尔的，要开，入仓不要充电，要等霍尔合盖。
  - 具体原理暂时不知道
- 一般的就是100K，入仓直接充电

## 打开智能仓把带入仓检测功能的仓的灯效搞没了

- 客户明说是普通仓的话就不能开智能仓功能。
  - 普通仓有入仓检测功能，耳机需要做对应修改。
...
```c
#if   DHF_EAR_INBOX_DECTET_ENABLE
bool user_flag = 1;
u8 power_on_5v_off_flag = 0;
u8 power_on_full_flag = 0;


#endif
#if DHF_EAR_INBOX_DECTET_NOPOWEROFF_ENABLE
bool user_flag = 1;
#endif
static int bt_connction_status_event_handler(struct bt_event *bt)
//...    
...
#if   DHF_EAR_INBOX_DECTET_ENABLE
        power_on_5v_off_flag = 1;
        int read_res = syscfg_read(CFG_USER_CHARGE_FULL_FLAG,&power_on_full_flag,1);
        if (power_on_full_flag) {
            power_on_full_flag = 0;
            user_flag = 1;
            int write_res = syscfg_write(CFG_USER_CHARGE_FULL_FLAG,&power_on_full_flag,1);
        }
#endif
```
...
#if   DHF_EAR_INBOX_DECTET_ENABLE
        user_charge_full_flag = 1;
        int ret = syscfg_write(CFG_USER_CHARGE_FULL_FLAG, &user_charge_full_flag, 1);
        extern bool user_flag;
        user_flag = 0;
...
#endif
    #if   DHF_EAR_INBOX_DECTET_ENABLE
    extern u8 power_on_5v_off_flag;
     if (!power_on_5v_off_flag) {
            extern bool user_flag;
            user_flag = 1;
     }
    #endif
