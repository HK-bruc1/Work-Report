...
static void anc_mode_switch_deal(u8 mode)
{
    ...
    if (anc_hdl->state == ANC_STA_OPEN) {
        ...
        if (anc_hdl->param.mode != ANC_OFF) {
            /*anc工作，关闭自动关机*/
            //sys_auto_shut_down_disable();这里也要注释掉，不然定时器还是会被删除
...
- `__ctmu_short_click_time_out_handle`
...
```c
    ...
    case 5:
        ...
        break;
    case 6:
        e.u.key.event = KEY_EVENT_SIXTH_CLICK;
        break;
    default:
...
六击写了处理但是没有作用，优先考虑按键扫描不支持。需要扩展。

## 同时多击按键？？？

添加两个全局变量标志位,感觉不写这里也可以。
...
```c
    ...
    volatile u8 local_click_press;//本地按键点击标志
    volatile u8 remote_click_release;//对耳按键释放标志
} APP_VAR;
```
...
```c
u16 timer_flag = 0;
static void clean_local_remote_click_flag(void)
{
    app_var.local_click_press = 0;
    app_var.remote_click_release = 0;
    sys_timeout_del(timer_flag);
} 
...
int app_earphone_key_event_handler(struct sys_event *event)
{
    case KEY_FACTORY_RESET:
        if((u32)event->arg == KEY_EVENT_FROM_TWS){
            app_var.remote_click_release = 1;
        }else {
            //是本地消息
            app_var.local_click_press = 1;
        }
        //超时全部清除标志位
        if(timer_flag == 0){
            //第二次就不会进入了
            timer_flag = sys_timeout_add(NULL, clean_local_remote_click_flag, 1000);
            break;
        }
        if(app_var.remote_click_release && app_var.local_click_press){
            //同时五击事件触发
            log_info("KEY_FACTORY_RESET");
            //if(get_bt_connect_status() < BT_STATUS_CONNECTING){
            tone_play(TONE_RESET_VOL,1);
...
- **这么一看的话，按键处理似乎是主机处理？从机接收到就发给主机？不然这个无法解释。**

## 添加滑动触摸以及触摸按键

![image-20251125110901306](./开发问题.assets/image-20251125110901306.png)

![image-20251125110942541](./开发问题.assets/image-20251125110942541.png)

```c
#define TCFG_LP_TOUCH_KEY0_EN    			1                  		//是否使能触摸按键0 —— PB0
#define TCFG_LP_TOUCH_KEY1_EN    			0                  		//是否使能触摸按键1 —— PB1
...
![image-20251125111114716](./开发问题.assets/image-20251125111114716.png)

![image-20251125111200523](./开发问题.assets/image-20251125111200523.png)

```c
#define TCFG_LP_TOUCH_KEY0_EN    			1                  		//是否使能触摸按键0 —— PB0
#define TCFG_LP_TOUCH_KEY1_EN    			0                  		//是否使能触摸按键1 —— PB1
#define TCFG_LP_TOUCH_KEY2_EN    			1                  		//是否使能触摸按键2 —— PB2
#define TCFG_LP_TOUCH_KEY3_EN    			1                  		//是否使能触摸按键3 —— PB4
#define TCFG_LP_TOUCH_KEY4_EN    			0                  		//是否使能触摸按键4 —— PB5
...
//两个按键以上，可以做简单的滑动处理
#define TCFG_LP_SLIDE_KEY_ENABLE            1                       //是否使能触摸按键的滑动功能
#define TCFG_LP_SLIDE_KEY_SHORT_DISTANCE    1                       //两个触摸按键距离是否很短，只支持两个按键，现流行适用于耳机。如果距离很远，则可以两个以上的远距离触摸按键的滑动。
...
- 既有默认触摸，也有新增的两个触摸脚位。使用多个触摸按键作为滑动触摸按键使用。
- 开启触摸按键音会所有触摸失效。

```bash
[00:13:03.612][Debug]: [LP_KEY]CH2: FALLING
[00:13:03.613][Debug]: [LP_KEY]falling_res_avg: 7699
[00:13:03.693][Debug]: [LP_KEY]CH2: RAISING
[00:13:03.693]CH2: key_type = 0xff
[00:13:05.234][Debug]: [LP_KEY]CH3: FALLING
[00:13:05.235][Debug]: [LP_KEY]falling_res_avg: 8146
[00:13:05.370][Debug]: [LP_KEY]CH3: RAISING
[00:13:05.371]CH3: key_type = 0xff
[00:13:45.675][Debug]: [LP_KEY]CH1: FALLING
[00:13:45.676][Debug]: [LP_KEY]falling_res_avg: 7614
[00:13:45.832][Debug]: [LP_KEY]CH1: RAISING
[00:13:45.833]CH1: key_type = 0xff
```

- 先保证各触摸点位都有触摸
- 在按照生产去装机，抓每个点位的触摸数据进行校准才行。
  - 没有介质，靠裸板手动很难触发。是一个连体硬件。
  - 两个点位在很短时间内同时按下。

![image-20251126102214592](./开发问题.assets/image-20251126102214592.png)
...
怎么实开启一个AD，就可以实现三个实体按键分别控制？通过电压值（电阻值）

## 带APP的按键配置

`apps\common\third_party_profile\jieli\JL_rcsp\adv_rcsp_protocol\rcsp_adv_bluetooth.c`
...
struct t_s_info _s_info = {
    ...
    .key_setting = {
    //左右通道01左/02右，按键事件，按键消息 16进制写法，不然APP显示有问题。
        0x01, 0x01, 0x05, \
        0x02, 0x01, 0x05, \
        0x01, 0x02, 0x03, \
        0x02, 0x02, 0x04,\
...
- **这个名称当选中下拉框中的名称时，输出的提示音路径以及名称是无法修改的。**

### 提示音太长导致播报有问题

- 默认来电提示音，两秒都异常，时长参考其他提示音长度
...
if ((get_second_call_status() == BT_THREE_CALL_INCOMING) || (get_second_call_status() == BT_THREE_CALL_COMING)) {
    ...
    //第一次来电会挂起，如果是挂起的话就会切换。首先需要满足条件可以进入。需要两个条件
```
...
![image-20251203101737950](./开发问题.assets/image-20251203101737950.png)

![image-20251203104226609](./开发问题.assets/image-20251203104226609.png)
...
```c
BT_MUSIC_STATUS_SUSPENDING==a2dp_get_status()
```
...
```c
#define DAC_PA_EN         1//0
#define DAC_PA_PORT       IO_PORTC_02


#define DAC_ANALOG_OPEN_PREPARE     1//开dac前
#define DAC_ANALOG_OPEN_FINISH      2//开dac后
#define DAC_ANALOG_CLOSE_PREPARE    3//关dac前
#define DAC_ANALOG_CLOSE_FINISH     4//关dac后
...
![image-20251119110429876](./开发问题.assets/image-20251119110429876.png)

# 入耳检测

## 外置的硬件

![image-20251126103122145](./开发问题.assets/image-20251126103122145.png)
...
注意点：
  1：晶振选型，参数必须要严格遵守原理图上标注的规格（参考下图一）
  2：软件调频偏，使用void bt_osc_offset_set(s32 offset);来配置。需要在蓝牙初始化完BT_STATUS_INIT_OK事件调用。
  若需要软件调频偏后，需要支持测试盒或者第三方仪器校准值覆盖，需要在BT_STATUS_INIT_OK事件设置频偏时先判断是否存在校准值。（参考下图二）
  3：第三方工具调整频偏，使用void bt_osc_offset_set(s32 offset);配置后，使用void bt_osc_offset_save(void);保存。
  调频偏的方法：
    a：软件程序调用API调整频偏。
    b：使用杰理工具调整频偏（测试盒/烧写器）。
    c：使用使用第三方工具调整频偏。
  兼容场景：
  场景一：a、b、c的其中一种
  场景二：a+b
  场景三：a+c
  注：以上情况调整平片方式，适用于耳机可视化的全系列芯片。
  图一：晶振选型

![92af95a2c27401cf602ab026fd374638](./开发问题.assets/92af95a2c27401cf602ab026fd374638.png)

图二：软件调整频偏

![617945c1223b8bad8fe297ffc7d21f01_0](./开发问题.assets/617945c1223b8bad8fe297ffc7d21f01_0.png)
...
![image-20251215113400205](./开发问题.assets/image-20251215113400205.png)

可视化工具中配置来电的提示音。同步铃声的优先级高于默认来电提示音。

# RCSP

## 打开寻找设备模块

- `apps\common\third_party_profile\jieli\JL_rcsp\bt_trans_data\le_rcsp_adv_module.h`
...
#define RCSP_ADV_MUSIC_INFO_ENABLE      0//1
#define RCSP_ADV_HIGH_LOW_SET			0//1
#define RCSP_ADV_FIND_DEVICE_ENABLE     1 //打开
...
//音量增强到最大
#define TCFG_AUDIO_DAC_ENABLE				ENABLE_THIS_MOUDLE
#define TCFG_AUDIO_DAC_LDO_VOLT				DACVDD_LDO_2_80V //普通模式 DACVDD
#define TCFG_AUDIO_DAC_LDO_VOLT_HIGH        DACVDD_LDO_2_80V //音量增强模式 DACVDD
#define TCFG_AUDIO_DAC_DEFAULT_VOL_MODE     (1) //首次开机默认模式 1：音量增强模式  0：普通模式
#define TCFG_AUDIO_DAC_VCM_CAP_EN           (1) //VCM引脚是否有电容  0:没有  1:有
```

如果带ANC的话，`cpu\br36\audio\audio_config.h`
