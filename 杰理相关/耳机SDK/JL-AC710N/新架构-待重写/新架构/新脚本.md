...
REM 生成条件编译宏（空定义）
echo #define _%1>apps\earphone\board\br56\customer.h

REM 使用PowerShell处理配置文件
powershell -Command "Select-String -Path 'apps\earphone\board\br56\customer.conf' -Pattern '^_.*=' | ForEach-Object { $parts = $_.Line -split '=', 2; '#define ' + $parts[0].Trim() + ' ' + $parts[1].Trim() } | Add-Content 'apps\earphone\board\br56\customer.h'"
```
