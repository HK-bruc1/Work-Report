...
```c
#define CTMU_SHORT_CLICK_DELAY_TIME 	400 	//单击事件后等待下一次单击时间(ms)
#define CTMU_LONG_CLICK_DELAY_TIME 		2000 	//产生长按事件的触摸时间(ms)
#define CTMU_HOLD_CLICK_DELAY_TIME 		200 	//long事件产生后, 发hold事件间隔(ms)

#define CTMU_SHORT_LONG_DELAY_TIME      1000// (CTMU_SHORT_CLICK_DELAY_TIME*3)   //多击长按时长按的时间
...
static void ctmu_long_click_handle(void)
{
   ...
   /* if( (e.u.key.event == KEY_EVENT_LONG) && (e.u.key.value == 0)) {
        call_event_ke_long_call_hangup();
    }*/
}
