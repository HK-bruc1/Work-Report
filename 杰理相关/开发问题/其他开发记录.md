...
这么来看的话，只要在`apps\earphone\board\br36\board_ac700n_demo.c`中的 `.max_vol = IDEX_TONE_MAX_VOL,`不是配置`IDEX_TONE_NONE`的话。那么最大最小提示音都会同步播放。当各自触发时再播放具体的提示音。

现在客户只要最大提示音。所有有关`TONE_MIN_VOL`的地方全部改成`IDEX_TONE_NONE`
...
void volume_down(u8 dec)
{
        ...
        if (get_tws_sibling_connect_state()) {
            ...
            //     bt_tws_play_tone_at_same_time(SYNC_TONE_MAX_VOL, 400);//这里共享的是最大音量的。可以改成SYNC_TONE_MIN_VOL
...
#endif
        {
            ...
            STATUS *p_tone = get_tone_config();
            //tone_play_index(IDEX_TONE_MIN_VOL, 1);
            //tone_play_index(IDEX_TONE_NONE, 1);
#endif
        }
```
...
- 改了这里没用，因为共享的是`SYNC_TONE_MAX_VOL`那边要修改一下。
...
![image-20250425091528271](./其他开发记录.assets/image-20250425091528271.png)
...
![image-20250708144128914](./其他开发记录.assets/image-20250708144128914.png)
...
![image-20250708144941942](./其他开发记录.assets/image-20250708144941942.png)
