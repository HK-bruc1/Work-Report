...
![image-20260303103546757](./谷歌快连.assets/image-20260303103546757.png)
...
- 换一个`cpu\br56\liba\google_fps.a`静态库
  - 可以实现单耳另一边呈现灰色，可以执行查找耳机命令
- 提示音文件格式
  - 如果是wav格式的话，那么保持原格式，转换后会失效，异常
    - RCSP以及涂鸦正常，但是GFPS不行
  - 如果是map3的话，万金油格式随便转换，音质可能有损耗。

# 无法弹窗但是在自测APP上能看到对应的MODEID

validated显示never

1. 开启VPN环境
2. 公钥，私钥，模型ID没有问题，一般就OK
