...
![image-20250609164507645](./问题收集.assets/image-20250609164507645.png)

- 这个是外部的。
- 默认16最大
  - 可以开启32级音量

![image-20250609164542976](./问题收集.assets/image-20250609164542976.png)
...
#define BT_A2DP_DEFAULT_VOLUME          16//VOL_MAX/2   //设置非IOS设备默认音量，根据系统音量级数设置，需要打开BT_A2DP_DEFAULT_VOLUME_EN
#define BT_A2DP_IOS_DEFAULT_VOLUME      16//VOL_MAX/2   //设置IOS设备默认音量，根据系统音量级数设置，需要打开BT_A2DP_DEFAULT_VOLUME_EN
```

### 换提示音

最大小音量在可视化工具中只有两个选项，无提示音以及电子音

- 选无提示音的话，优先获取资源文件的外部提示音，再获取SDK中自带的中英文外部提示音
- 选电子音的话，就是内部电子音。

换外部的话，只能同名替换了。内部电子音没办法替换。

### 使用外部来电提示音

在配置中把来电报号关闭，播放手机铃声关闭：

![image-20250715121246762](./问题收集.assets/image-20250715121246762.png)

傻逼。就无提示音和tone音（这是内部的。。。）

### 问题

安卓手机尤其是在点开了微信界面，有时候点3下左耳启动siri的时候是没有“叮”的一声。
...
```c
sys_cb1.zhuer_together_flag = 0;//主机触发五击事件标志
sys_cb1.fuer_together_flag = 0;//从机触发五击事件标志
sys_cb1.together_mun_flag = 0;//是否开始计时的标志并作为超时阈值使用
```

- 这三个变量应该是各自耳机的全局变量
- 不是TWS中公用的全局变量

在这里定义`platform\bsp\bsp_sys.h`
...
#if UDEF_TWS_CONN_LED_DEAL_EN
    u8 phone_conn_led_update_flag;       //手机连接刷新状态灯
    u8 phone_discon_led_update_flag;     //手机断开刷新状态灯
#endif

#if DHF_KEY_CLICK_TOGETHER				//使用宏包含
    u8 together_mun_flag;
    u8 fuer_together_flag;
    u8 zhuer_together_flag;
#endif
} sys_cb1_t;
extern sys_cb1_t sys_cb1;
```

**利用定时器不断轮询耳机中三个变量的状态，超时状态就重置，避免两个耳机操作时间相差过大也触发**

`platform\functions\func_bt.c`
...
{
    ......
#if DHF_KEY_CLICK_TOGETHER
    //这里主从都会进入，因为各自进入五击时，together_mun_flag被设置为1了,意味着超时机制开始
    //无论是主机还是从机各自触发五击事件后，都会把各自耳机的计时标志设置为1，感觉前面两个条件没啥必要性
    if((sys_cb1.zhuer_together_flag==1)||(sys_cb1.fuer_together_flag== 1)||(sys_cb1.together_mun_flag >= 1))
    {
        //检测到了五击击信号或者清理标志位的计时已开启，开始计时
        sys_cb1.together_mun_flag++;//计时
    }
    //如果5ms的计时器到达100次(500ms)，则清除同时按下标志位和计时标志位
    //那么对应的同时五击事件不满足条件就不会被触发了
    if(sys_cb1.together_mun_flag >= 100)
    {
        sys_cb1.zhuer_together_flag = 0;
        sys_cb1.fuer_together_flag = 0;
        sys_cb1.together_mun_flag = 0;
    }
#endif
}
```

**把清除配对信息的操作封装为一个消息事件来处理**

`platform\bsp\bsp_key.h`
...
#if UDEF_PROJ_UI_EN
#define EVT_CLEAN               0x7ca

#if DHF_KEY_CLICK_TOGETHER
#define EVT_TOGETHER_MSG_EN    0x7c9
#endif
...
void func_message(u16 msg)
{
    switch (msg) {
        ...
        case KL_PREV_VOL_UP:
            ......
#if DHF_KEY_CLICK_TOGETHER
        case EVT_TOGETHER_MSG_EN:
            //led_set_sta(0xA8,0x00,0x04,0xff);
...
`projects\earphone\message\msg_bt.c`中`func_bt_message_do`
...
```c
        ...
        if(bt_nor_is_connected() && bt_tws_is_connected()){
            ...
            if(bt_tws_is_slave()){
                //从机触发了五击事件，超时机制开始
                sys_cb1.together_mun_flag = 1;
                //发消息告诉主机，从机已经触发五击了，修改主机的全局变量sys_cb1.fuer_together_flag = 1;
                bt_tws_user_key(0x16);
            } else {
                //主机触发了五击事件，超时机制开始
                sys_cb1.together_mun_flag = 1;
                //主机五击标志位设置为1，初始全部变量是0
                //直接设置1覆盖就行，多此一举
                if(sys_cb1.zhuer_together_flag == 0)
                    sys_cb1.zhuer_together_flag = 1;
            }
        }
#endif      
```
...
#if DHF_KEY_CLICK_TOGETHER
    //只有主机能进入
    //主机进入五击的话，zhuer_together_flag会被设置为1
    //副机进入五击的话，通过自定义键消息给主机，主机的fuer_together_flag会被设置为1
    //主机和副机都进入五击的话，together_mun_flag会被设置为1
    if((sys_cb1.zhuer_together_flag==1)&&(sys_cb1.fuer_together_flag== 1)&&(sys_cb1.together_mun_flag <= 100))
	{
		bt_tws_user_key(0x17);//给副耳发送消息，执行清理消息
        //主机执行同步关机操作
		func_message(EVT_TOGETHER_MSG_EN);//主机执行清理消息
	}
#endif
}
```
...
`platform\bsp\bsp_bt.c`中`bt_emit_notice`

```c
#if DHF_KEY_CLICK_TOGETHER
        else if(param == 0x16){
            //把主机的从机五击按键标志位设置为1
            sys_cb1.fuer_together_flag = 1;
        } else if(param == 0x17) {
            //给对耳发送TOGETHER_MSG_EN消息
            //func_message(EVT_TOGETHER_MSG_EN);
            //为了同步关机从机这里直接消除即可
            bt_clr_all_link_info('k');
        }
#endif
...
```c
    ...
    case KSE_PLAY_PWR_USER_DEF:
        ...
        if (!bt_nor_is_connected()) {
            bt_tws_user_key(0x16);//给对耳发送信息
            bt_clr_all_link_info('k');//清配手机和TWS对信息
            // 红灯亮1秒后自动熄灭
            led_set_sta(0xff, 0x00, 1, 20);
            sys_cb.pwrdwn_tone_en = 1; //提示音
            //同步关机
            sys_cb.discon_reason = 1;
            //把模式从蓝牙模式切换到关机模式 
...
`app\platform\bsp\bsp_bt.c`的`void bt_emit_notice(uint evt, u32 param)`
...
///五击按键处理
    ...
    case KFI_PLAY_PWR_USER_DEF:
        ...
        //delay_5ms(100);
        // bt_nor_disconnect(); //先手机断开连接
        bt_tws_user_key(0x16);//给对耳发送信息同步清除配对信息
        bt_clr_all_link_info('k');//如果连接状态下清除配对信息，连接不会掉，需要重启蓝牙才会重新连接。
        //关机
        sys_cb.pwrdwn_tone_en = 1; //关机提示音
        sys_cb.discon_reason = 1; //同步关机
        func_cb.sta = FUNC_PWROFF;
```

- 源码是非连接以及非TWS模式下的单耳清空配对信息
- 如果客户要求是连接状态下以及TWS模式下清空配对信息的话。
  - 似乎清除掉了保存的配对信息，但是连接没有断开。必须重启手机蓝牙，耳机回连记忆设备时，去读取保存的信息时，才会生效
  - **但是体验很不好，清除配对后，耳机竟然没有断开？有不有对应的函数可以使TWS耳机断开？**
  - 自己找`platform\libs\api_btstack.h`
  - **先与手机断开连接，在清除**
...
```c
            ...
            //bt_nor_disconnect(); //断开连接
            bt_clr_all_link_info('k');//TWS同步清除
        }
#endif
```
...
```c
#if DHF_NO_SLEEP_WITHOUT_CONN
    if(!bt_is_connected()){//在函数中可以加一个判断实现没连接就不进入低功耗
        return;
    }
#endif

#if UDEF_PROJ_UI_EN
	//功耗会增加，就算进入了`bt_enter_sleep();`不执行后面两个关灯函数，功耗还是会高一点。
    bt_enter_sleep();//没连接直接return不进入低功耗，连接后才进入低功耗，但是不关灯。
#endif
...
![image-20250703115950666](./问题收集.assets/image-20250703115950666.png)

**关机后会有，扬声器的问题，看看参数。调回公版果然没有了。有一些参数是没有的。有一些参数有。**

### 整体音量加减时的喇叭输出的步进幅度

- 比如处在一个很低的音量等级上，但是声音却很大。

![image-20250619195605895](./问题收集.assets/image-20250619195605895.png)

[可能会导致的问题](###关机提示音后有“波”的杂音)

- 还可以通过音量表`platform\bsp\bsp_dac.c`
- 实现在参考的SDK上

```c
#if DHF_UDEF_DAC_DVOL_16_EN
AT(.rodata.dac)
const u8 usr_dac_dvol_tbl_16[16 + 1] = {
    56,53,49,46,42,39,35,32,28,25,21,18,14,11,7,4,0,
};
#endif
```
...
![image-20250704142351137](./问题收集.assets/image-20250704142351137.png)

### 谁在生效

两个都要开。代码决定执不执行功能，工具决定参数。工具是NTC关机，代码是充电NTC保护。

这个不用管。

```c
#define USER_NTC                        0//1           //NTC ADC检测
...
{
    static int chg_en_cnt = 0;      // 充电使能计数器
    static int chg_dis_cnt = 0;     // 充电禁止计数器

    // 检查NTC使能和充电使能标志
    if (xcfg_cb.sys_ntc_en && xcfg_cb.charge_en) {
        
        // 当NTC温度值低于或等于阈值时（温度过高或过低异常）
        if (adc_cb.ntc_val <= CHARGE_NTC_DETECT_VAL) {
            ...
            if (chg_dis_cnt < 20) {
                ...
                chg_dis_cnt = 30;   // 防止重复执行
                
                // 如果当前正在充电且5V电源接入
                if(sys_cb.charge_sta == 1 && CHARGE_DC_IN()) {
                    RTCCON8 = (RTCCON8 & ~BIT(6)) | BIT(1);     // 禁用充电器功能
                    sys_cb1.chg_ntc_stop = 1;                   // 设置NTC停止充电标志
                ...
                // 如果没有5V接入，设置关机标志
                    ...
                    sys_cb.ntc_2_pwrdwn_flag = 1;              // NTC异常关机标志
                }
            }
            chg_en_cnt = 0;         // 清零充电使能计数器
        } 
        // 当NTC温度值高于阈值时（温度正常）
        else {
            ...
            if (chg_en_cnt < 20) {
                ...
                chg_en_cnt = 30;    // 防止重复执行
                
                // 如果5V接入且当前未在充电状态，恢复充电
                if(CHARGE_DC_IN() && sys_cb.charge_sta == 0){
                    RTCCON8 = (RTCCON8 & ~BIT(1)) | BIT(6);     // 启用充电器功能
                    charge_stop(0);                             // 进入充电状态
                    sys_cb1.chg_ntc_stop = 0;                   // 清除NTC停止充电标志
                    //printf(charge_ntc_check_str);
                }
            }
            chg_dis_cnt = 0;        // 清零充电禁止计数器
        }
    }
}
#endif
...
![image-20250704150219921](./问题收集.assets/image-20250704150219921.png)

![image-20250704150510753](./问题收集.assets/image-20250704150510753.png)
...
- **UI没有明确要求pairing的话，安全起见把pairing的提示音关掉。**
  - 在工具中关配对提示音
  - 不行再把`projects\earphone\config.h`
    - `\#define WARNING_BT_PAIR         0//1       //BT PAIRING提示音`

### 低电提示与低电关机不要同步

**TWS连接后，低电提示音和关机一般都是同步的，客户要求不要同步。**

- 意思就是那个耳机低电导致关机了，就关单耳。不要同步
- 低电时关机各关各的。

![image-20250626122927717](./问题收集.assets/image-20250626122927717.png)
...
#define BT_DUT_TEST_EN                  1//0   //蓝牙DUT测试使能-----DUT也是一个应用模式FUNC_BT_DUT，需要使能宏定义
    ...
    case KSI_PLAY_PWR_USER_DEF:
        ...
        //TWS自定义按键,通过bt_tws_user_key函数发送,bt_emit_notice函数处理
    case BT_NOTICE_TWS_USER_KEY:
        if (param == TWS_USER_KEY_RING_STOP) {
            f_bt.ring_stop = 1;
        } else if (param == TWS_USER_KEY_ATMOS) {
...
`app\platform\bsp\bsp_bt.c`的`hfp_get_bat_level_hook`

该函数`hfp_get_bat_level_hook`是蓝牙电量获取的钩子函数，主要作用是在启用TWS（真无线立体声）模式时同步双耳电量显示。
...
void hfp_get_bat_level_hook(uint *level)
{
    ...
    f_bt.loc_vbat = *level;  // 保存本地电量值
    if(bt_tws_is_connected() && (bt_tws_is_slave() == 0)){  // 主耳且TWS连接时
        if((*level > f_bt.rem_vbat) && (f_bt.rem_vbat != 0xff)){  // 本地电量高于从耳且从耳电量有效
            *level = f_bt.rem_vbat;  // 强制显示从耳电量
        }
    }
#endif
    // ... existing code ...
...
1. **同步逻辑缺陷**：主耳会强制显示从耳电量（当本地电量更高时），可能导致：
   - 从耳未正确上报电量（如初始化值0xff未更新）
   - 双耳电量同步通道异常（如TWS通道未正确握手）
2. **显示策略问题**：主耳放弃自身电量显示可能导致：
   - 从耳电量传感器故障时显示异常
   - TWS连接断开时未及时恢复本地电量显示
...
#if Y90_UI_EN
AT(.com_rodata)
const u16 usr_tbl_bat_level[10] = {
    3000,   //0 [0%~10%]         //3200关机电压, 不可能运行到这个电压, 运行到这个电压就关机了,这个默认不要动
    3613,   //1 [10%~20%]
    3690,   //2 [20%~30%]
    3729,   //3 [30%~40%]
    3749,   //4 [40%~50%]
...
void hfp_get_bat_level_hook(uint *level)
{
    //*level = 2;
    uint bat_level = 0;
    while ((bat_level < 10) && (usr_tbl_bat_level[bat_level] < sys_cb.vbat)) {
        bat_level++;
    }
    if (bat_level) {
        bat_level--;
    ...
    /*
        ...
        bat_level = bt_get_bat_level(10);
        if (bat_level > 9) {
            bat_level = 9;
        }
    }
    */
    *level = bat_level;                     //fix 手机上蓝牙电量对应的级数
}
...
修改后的代码与原始代码在功能上有较大的变化，尤其是在电量等级映射和电量获取逻辑方面。下面我将从**功能差异、稳定性影响、用户体验**三个方面进行详细对比分析。

| 功能点               | 原始代码                                                     | 修改后代码                                                   |
| :------------------- | :----------------------------------------------------------- | :----------------------------------------------------------- |
| **电量映射方式**     | 没有自定义电压到电量级数的映射表                             | 新增了 `usr_tbl_bat_level` 表，用于将电池电压映射为 0~9 级（10% 一级） |
| **电量处理逻辑**     | 仅注释掉一个固定值赋值：`*level = 2;`，未实际处理电量        | 实际根据电压查表计算出对应的电量等级，并赋值给 level         |
| **是否启用 UI 定制** | 不涉及 Y90_UI_EN 宏控制                                      | 使用宏 `Y90_UI_EN` 控制是否启用该电量映射逻辑                |
| **TWS 同步逻辑**     | 原函数中包含 TWS 主从同步逻辑（主耳显示从耳电量）            | 修改后完全移除 TWS 相关同步逻辑                              |
| **备用逻辑**         | 注释掉一个备用逻辑：[bt_get_bat_level(10)](javascript:void(0)) 获取默认值 | 移除了备用逻辑，只依赖电压查表结果                           |

| 影响项               | 描述                                                         |
| :------------------- | :----------------------------------------------------------- |
| **电压读取准确性**   | 函数使用 `sys_cb.vbat`，需确认其单位是 mV 还是原始 ADC 值。如果单位不匹配，可能导致电量映射错误 |
| **数组越界风险**     | `while ((bat_level < 10) && (usr_tbl_bat_level[bat_level] < sys_cb.vbat))` 中，如果 `sys_cb.vbat > 4047`，会导致 `bat_level++` 越界（建议添加边界判断） |
| **TWS 电量同步失效** | 原函数中主耳会同步从耳电量，避免双耳电量显示不一致；修改后完全取消此逻辑，可能导致 TWS 双耳显示电量不同步 |
| **默认电量缺失**     | 如果电压低于最低阈值（3000），`bat_level` 会为 0，但此时可能已是低电状态，缺少低电提示或强制关机机制 |

| 用户体验                          | 描述                                                         |
| :-------------------------------- | :----------------------------------------------------------- |
| **优点 - 更精确的电量展示**       | 使用查表法可以更精细地划分电量等级（每10%一级），提升用户对剩余电量的感知精度 |
| **缺点 - 缺乏异常处理**           | 若电压超出查表范围（过高或过低），没有兜底策略，可能导致显示异常 |
| **UI 一致性增强（若配合 UI 层）** | 配合 `Y90_UI_EN` 宏可实现定制化电量图标显示，提高产品差异化能力 |
| **TWS 用户体验下降**              | 移除了 TWS 同步逻辑，可能导致左右耳电量显示不一致，尤其是当一只耳机电量较低时仍显示高电量 |
...
```c
这里有十个电量百分比的临界值,但是根据放电曲线图来看,有一些曲线长有一些曲线短,我需要长度平均一点,那么这个临界值就要对应调整:
const u16 usr_tbl_bat_level[10] = {
    3000,   //0 [0%~10%]         //3200关机电压, 不可能运行到这个电压, 运行到这个电压就关机了,这个默认不要动
    3563,   //1 [10%~20%]
    3650,   //2 [20%~30%]
...
10%-0%:占用1格时间曲线
其余阶段曲线持续时间不足一格
你要确定好临界值，因为你改动一个值，影响的是两个百分比放电时间段
```

### 实践总结

似乎全部临界值全部加0.1V效果就可以均匀了。

### 电量计算

![image-20251115173911452](./问题收集.assets/image-20251115173911452.png)
...
![image-20250925094406486](./问题收集.assets/image-20250925094406486.png)

![image-20250925094428844](./问题收集.assets/image-20250925094428844.png)

只留下必要部分

![image-20250925094608237](./问题收集.assets/image-20250925094608237.png)

计算

![image-20251115173955890](./问题收集.assets/image-20251115173955890.png)
...
无论如何也走不到手动配对啊？在判断条件中就已经执行了，看他返回是什么了？

- 返回1就不会进入手动尝试配对
- 执行完后，返回不是1的话，就会尝试进入手动配对
  - 比如工具中配置双击无操作的话，就会返回0
...
///五击按键处理
    ...
    case KFI_PLAY_PWR_USER_DEF:
        ...
        tws_res_play(TWS_RES_TONE);//双耳同步嘟一声
        delay_5ms(100);
        if(bt_tws_is_connected() && bt_is_connected()){
            bt_tws_user_key(0x16);
            bt_clr_all_link_info('k');
            sys_cb.pwrdwn_tone_en = 1; //关机提示音
            sys_cb.discon_reason = 1; //同步关机
...
- `tws_res_play`提示音与关机和DUT模式时，老是播报一边，不知道为什么。

## 自定义灯效

`void led_set_sta(u8 rled_sta, u8 bled_sta, u8 unit, u8 period);`

**`rled_sta`** - 红灯8个时间片的开关状态

- 8位二进制，每一位对应一个时间片的开/关

**`bled_sta`** - 蓝灯8个时间片的开关状态
...
**`period`** - 8个时间片组成一个完整周期后的重复间隔时间

- 一个完整的8时间片模式执行完后，等待多长时间再重复

```c
led_set_sta(0x00, 0x60, 0x04, 0x19);
//红灯：8个时间片全关闭
//蓝灯：第6、7时间片点亮（01100000）
//每个时间片持续4个时间单位
//8个时间片执行完后，等待25个时间单位，然后重复这个模式
//这样就能创建各种复杂的LED闪烁节奏和模式了！
```

## 硬件分左右

![image-20250620191025820](./问题收集.assets/image-20250620191025820.png)

![image-20250620191052112](./问题收集.assets/image-20250620191052112.png)

如果按键操作相反的话，直接在配置工具配置。

如果左右耳按键不一样的话，就要在代码中区分了。
...
if(bt_tws_is_connected()) {
    ...
    if(left_channel) {
        ...
        //不接地的就按右声道处理，进入低延迟模式,他会自己切换
        user_def_key_msg(UDK_GAME_SWITCH);
    }
}
```
...
`led_set_sta`这种方式似乎需要修改的地方很多。

```c
case BT_STA_IDLE:
#if DHF_TWS_PAIRING_LED
#if UDEF_PROJ_UI_EN
        if(bt_tws_is_connected() && left_channel) {
            led_bt_connected();
        } else {
            led_bt_idle();
        }
...
#if DHF_TWS_PAIRING_LED
#if UDEF_PROJ_UI_EN
        if(!bt_nor_is_connected() && !left_channel) {
            led_bt_idle();
            break;
        }
#else
...
这两个接口获取的是哪里的灯效？配置工具的？

- 是的
- `bt_idle`接口不知道是工具中哪一个状态的灯效。回连还是配对状态？
  - 这两个灯效一般保持一致
- TWS配对后，主耳还是配对状态（`bt_idle？`），但是副耳已经是`bt_connected`状态了。
  - 连接手机后主耳朵才会到连接状态。

前者是直接设置灯效了，后者是获取灯效配置。
...
![image-20250709165603352](./问题收集.assets/image-20250709165603352.png)

![1c0d2ae0d512c05a3662c62f4db4386](./问题收集.assets/1c0d2ae0d512c05a3662c62f4db4386.png)

![image-20250709170216150](./问题收集.assets/image-20250709170216150.png)

## 烧录问题

![image-20250715094538108](./问题收集.assets/image-20250715094538108.png)

存储超出了。有循环提示音

![image-20250715094843847](./问题收集.assets/image-20250715094843847.png)

## 蓝牙地址MAC地址

如果不是随机，而已分配到每一台机器的话，是需要计算的。有多少台机器，地址从哪里到哪里。

- 如果随机的话，所有的配置都要与原来随机的一样，不然工具报错。
  - 动了原来的随机配置的话，改回原来随机配置以及其他原来配置。
...
#define USER_LED_SLEEP_EN               0           //自定义休眠同步闪灯--------避免休眠模式时灯效被关闭
#define BT_TWS_SLEEP_LED_SYNC_EN        0           //TWS休眠同步闪灯

#define USER_PWM_LED_EN                 0           //是否开启PWM控制LED，可以修改LED亮度
#define USER_PWM_LED_LEVEL              5           //修改PWM控制LED亮度，10级可调，这是PWM亮度调节
...
```c
#define USER_LED_GPIO                   1                             //是否开启自定义修改LED的IO上拉电阻，会占用公共区
```

`app\platform\bsp\bsp_led.c`
...
![image-20251222143951101](./问题收集.assets/image-20251222143951101.png)
